...
export async function addStats(
  ...
  if (userStats !== undefined) {
    const interaction = userStats.interactions.find(
...
export async function getStatsFrom(momentDay: moment.Moment) {
  ...
  const todayInteractions = mapValues(
    groupBy(todayInteractionsDuplicated, (interac) => interac.interactionType),
    (type) => sumBy(type, "amount"),
  );
  const uniqueUsers = uniqBy(todayStats, (stat) => stat.userId).map(
    (stat) => stat.userId,
  ).length;
